...
class Enum(object):
    ...
    def __init__(self, name, keys, **kwargs):
        ...
        if isinstance(keys, collections.abc.Mapping):
            if not multiKeys:
                reverse = dict([(v, k) for k, v in list(keys.items())])
            else:
                reverse = dict()
    ...
    def __repr__(self):
        return '%s(\n%s)' % (self.__class__.__name__, ',\n'.join([repr(v) for v in list(self.values())]))
    ...
    def values(self):
        ...
        if isinstance(self._values, collections.abc.Mapping):
...
#        raise EnumImmutableError(index)
    ...
    def keys(self):
        ...
        return [self._reverse[v] for v in list(self.values())]
