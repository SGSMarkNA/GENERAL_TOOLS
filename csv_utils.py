...
def find_last_valid_row(csv):
	largest_lenght = 0
	for i in reversed(range(len(csv))):
		r = csv[i]
...
class CSV_Grid(list):
	...
	def __init__(self,*args,**kwargs):
		...
		if isinstance(file_path,(unicode, str,System_Path.Path)):
	...
	def has_Sub_Grid(self,name):
		""""""
		return self._subgrids.has_key(name)
	
	#----------------------------------------------------------------------
	def iter_Sub_Grids(self,sort_by_name=True):
		""""""
		grid_names = self._subgrids.keys()
		if sort_by_name:
			grid_names.sort()
	...
	def update_subGrids(self):
		""""""
		for name,grid in self._subgrids.items():
